...
for seednum =1:len(seeds)
    ...
    if exist([results_path 'konvid1k_iv3_results_no_ft_' num2str(baseseed) '_no_shuffle.mat'],'file')
        load([results_path 'konvid1k_iv3_results_no_ft_' num2str(baseseed) '_no_shuffle.mat'])
    else
        rng(baseseed);
        ...
        [netLSTM, info, PLCC, SROCC, YPRED, YTEST] = trainlstm_no_shuffle(TrainVideos, ValidationVideos, TestVideos, Name, MOS, actsstruct, LSTMParameters);
        save([results_path 'konvid1k_iv3_results_no_ft_' num2str(baseseed) '_no_shuffle.mat'],'PLCC','SROCC','YPRED','YTEST','TrainVideos','ValidationVideos','TestVideos');
    end
    perf{seednum} = SROCC;
end
